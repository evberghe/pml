...
# read accurs from file
load("accurs.RData")
```

Note that, in order to estimate the accurady of the prediction, we used data that were not seen by the
training algorithm. Indeed, if we would have estimated our aaccuracy on the same data than the ones we
used for training, we more than likely would have over-estimated the accuracy.
...
Obviously, since we don't know the true categories for the test set, we can't calculate the accuracy.
...
my computing equipment is not up to this, so I've not been able to apply this procedure to the data at hand.
